        ...
        if not skip_labels:
          if not agnostic_mode:
            if classes[i] in category_index.keys():
              ...
              if class_name == 'upstair':
                print("Upstair is detected!!")
                #s.sendall(u'upstair')
                str_data = "upstair"
                s.send(str_data.encode())
                mixer.init()
                mixer.music.load(r'C:\Users\Nayan\Downloads\Music\beep-01a.mp3')
                mixer.music.play()
              elif class_name == 'downstair':
                print("Downstair is detected!!")
                str_data = "downstair"
                s.send(str_data.encode())
                mixer.init()
                mixer.music.load(r'C:\Users\Nayan\Downloads\Music\beep-02.mp3')
                mixer.music.play()
            else:
              class_name = 'N/A'
